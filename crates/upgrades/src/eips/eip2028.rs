...
//! * _beta_ - chain growth rate [1/s]: the rate at which new blocks are added to the heaviest chain.
...
//!
//! However, the delay function can be decomposed into two parts D = *`D_t`* + *`D_p`*, where _`D_t`_ is the delay caused by the transmission of the block and _`D_p`_ is the delay caused by the processing of the block by the node. Our model and tests will examine the effect of Calldata on each of _`D_t`_ and _`D_p`_, postulating that their effect is different. This may be particularly relevant for Layer 2 Scalability and for Stateless Clients (Rationales 2, 3 above) because most of the Calldata associated with these goals are Merkle authentication paths that have a large _`D_t`_ component but relatively small _`D_p`_ values.
//!
//! ## Test Cases
//! To suggest the gas cost of calldata we shall conduct two types of tests:
//! 1. Network tests, conducted on the Ethereum mainnet, used to estimate the effect on increasing block size on _`D_p`_ and _`D_t`_, on the overall network delay D and the efficiency ratio (**), as well as delays between different mining pools. Those tests will include regression tests on existing data, and stress tests to introduce extreme scenarios.
